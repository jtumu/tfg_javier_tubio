...
Ecoli <- read.delim("s_cerevisiae_tabla.tsv")

####  Resultados FANTASIA

EColi_random100 <- read.csv("~/Desktop/TFG/Result_fantasia_Lite/S_Cerevisiae_random_111_resultados/Scerevisiae_random_111.csv")
EColi_random200 <- read.csv("~/Desktop/TFG/Result_fantasia_Lite/S_Cerevisiae_random_222_resultados/Scerevisiae_random_222.csv")
EColi_random300 <- read.csv("~/Desktop/TFG/Result_fantasia_Lite/S_Cerevisiae_random_333_resultados/Scerevisiae_random_333.csv")
EColi_random400 <- read.csv("~/Desktop/TFG/Result_fantasia_Lite/S_Cerevisiae_random_444_resultados/Scerevisiae_random_444.csv")
EColi_random500 <- read.csv("~/Desktop/TFG/Result_fantasia_Lite/S_Cerevisiae_random_555_resultados/Scerevisiae_random_555.csv")
EColi_control <- read.csv("~/Desktop/TFG/Result_fantasia_Lite/resultados_fantasiaLite_control/results_control_scerevisiae/results_control_scerevisiae.csv")

### Carpeta para guardar figuras 

carpeta_resultados <- "Figuras_TFG_SCerevisiae"
...
# if (!require("org.Sc.sgd.db", quietly = TRUE)) BiocManager::install("org.Sc.sgd.db")
...
write.csv(
  x = comparativa_h2_completa, 
  file = file.path(carpeta_resultados, "Comparativa_Aciertos_SCerevisiae.csv"),
  row.names = FALSE
)
...
write.csv(
  x = conteo_control, 
  file = file.path(carpeta_resultados, "Organismos_donadores_SCerevisiae_Control.csv"),
  row.names = FALSE
)
...
write.csv(
  x = resumen_org_randoms, 
  file = file.path(carpeta_resultados, "Organismos_donadores_SCerevisiae_Random.csv"),
  row.names = FALSE
)
...
########################
go_data_mf <- godata('org.Sc.sgd.db', ont="MF", computeIC=FALSE)
...
#  Filtrar el UniProt para la ontología que buscamos, primero MF
...
go_data_bp <- godata('org.Sc.sgd.db', ont="BP", computeIC=FALSE)
...
go_data_cc <- godata('org.Sc.sgd.db', ont="CC", computeIC=FALSE)
...
write.csv(
  x = distancias_similitud, 
  file = file.path(carpeta_resultados, "Wang_SCerevisiae_Randoms.csv"),
  row.names = FALSE
)
...
write.csv(
  x = distancias_similitud_control, 
  file = file.path(carpeta_resultados, "Wang_SCerevisiae_Control.csv"),
  row.names = FALSE
)
...
top_enriquecidos <- datos_enriquecimiento %>%
  ...
  ungroup() %>% 
  mutate(go_description_wrap = str_wrap(go_description, width = 37))
...
grafico_enriquecimiento <- ggplot(top_enriquecidos, 
                                  aes(x = Protein_Ratio, 
                                      y = reorder_within(go_description_wrap, Protein_Ratio, model_key), 
  ...
  labs(
    ...
    subtitle = "Términos sobre-predichos por la IA frente al proteoma real de S. Cerevisiae (UniProt)",
    x = "Protein Ratio", 
    y = ""
  ) +
  ...
  theme(
    ...
    plot.subtitle = element_text(hjust = 0.5, size = 15, margin = margin(b=15)),
...
ggsave(
  filename = file.path(carpeta_resultados, "Enriquecimiento_GO_Randoms_SCerevisiae.png"), 
  plot = grafico_enriquecimiento, 
  width = 12,                     
  height = 8,                     
  dpi = 300,                      
  bg = "white"                    
...
ggsave(
  filename = file.path(carpeta_resultados, "Enriquecimiento_GO_Control_SCerevisiae.png"), 
  plot = grafico_enriquecimiento_control, 
  width = 12,                     
  height = 8,                     
  dpi = 300,                      
  bg = "white"                    
...
go_data_mf_ic <- godata('org.Sc.sgd.db', ont="MF", computeIC=TRUE)
...
go_data_bp_ic <- godata('org.Sc.sgd.db', ont="BP", computeIC=TRUE)
...
go_data_cc_ic <- godata('org.Sc.sgd.db', ont="CC", computeIC=TRUE)
...
write.csv(
  x = tabla_ic_completa, 
  file = file.path(carpeta_resultados, "IC_SCerevisiae.csv"),
  row.names = FALSE
)
...
grafico_real_ontologia <- function(datos_ontologia, ontologia){
  ...
  ggplot(datos_grafico_real, aes(
    ...
    labs(
      title = paste0("Top 10 términos GO ",ontologia, " en el proteoma real de S. Cerevisiae"),
...
grafico_real_MF<-grafico_real_ontologia(ic_real_mf, "MF")
ggsave(
  filename = file.path(carpeta_resultados, "Top10GO_MF_Real_SCerevisiae.png"), 
  plot = grafico_real_MF, 
  width = 12,                    
  height = 8,                    
  dpi = 300,                    
  bg = "white"                    
...
grafico_real_BP<-grafico_real_ontologia(ic_real_bp, "BP")
ggsave(
  filename = file.path(carpeta_resultados, "Top10GO_BP_Real_SCerevisiae.png"), 
  plot = grafico_real_BP, 
  width = 12,                    
  height = 8,                    
  dpi = 300,                    
  bg = "white"                    
)
grafico_real_CC<-grafico_real_ontologia(ic_real_cc, "CC")
ggsave(
  filename = file.path(carpeta_resultados, "Top10GO_CC_Real_SCerevisiae.png"), 
  plot = grafico_real_CC, 
  width = 12,                    
  height = 8,                    
  dpi = 300,                    
  bg = "white"                    
...
grafico_control_ontologia <- function(datos_ontologia_control, ontologia){
  ...
  grafico_control_ordenado <- ggplot(datos_grafico_control, aes(
    ...
    geom_text(aes(label = veces_predicha), 
              hjust = -0.2, size = 7, color = "black") + 
    coord_flip() + 
    facet_wrap(~ model_key, scales = "free_y") + 
    scale_x_reordered() + 
    scale_fill_viridis_c(option = "plasma", name = "Nº Proteínas") +
    labs(
      title = paste0( "Top 10 términos GO ", ontologia ," en el control S. Cerevisiae"),
    ...
    theme(
      ...
      axis.text.y = element_text(lineheight = 0.8, size = 15),
      panel.grid.minor = element_blank())
  print(grafico_control_ordenado)
}
grafico_control_MF<-grafico_control_ontologia(ic_pred_mf_control, "MF")
ggsave(
  filename = file.path(carpeta_resultados, "Top10GO_MF_Control_SCerevisiae.png"), 
  plot = grafico_control_MF, 
  width = 12,                    
  height = 8,                    
  dpi = 300,                    
  bg = "white"                    
...
grafico_control_BP<-grafico_control_ontologia(ic_pred_bp_control, "BP")
ggsave(
  filename = file.path(carpeta_resultados, "Top10GO_BP_Control_SCerevisiae.png"), 
  plot = grafico_control_BP, 
  width = 12,                    
  height = 8,                    
  dpi = 300,                    
  bg = "white"                    
)
grafico_control_CC<-grafico_control_ontologia(ic_pred_cc_control, "CC")
ggsave(
  filename = file.path(carpeta_resultados, "Top10GO_CC_Control_SCerevisiae.png"), 
  plot = grafico_control_CC, 
  width = 12,                    
  height = 8,                    
  dpi = 300,                    
  bg = "white"                    
...
grafico_random_ontologia <- function (datos_ontologia_random, ontologia) {
  ...
  grafico_randomizados_ordenado <- ggplot(
    ...
    scale_fill_viridis_c(option = "plasma", direction = 1, 
                         ...
                         limits = c(1, 5))+
    labs(
      title = paste0("Top 10 términos GO ", ontologia, " en secuencias aleatorizadas S. Cerevisiae"),
...
grafico_random_MF<-grafico_random_ontologia(ic_pred_mf, "MF")
ggsave(
  filename = file.path(carpeta_resultados, "Top10GO_MF_Random_SCerevisiae.png"), 
  plot = grafico_random_MF, 
  width = 12,                    
  height = 8,                    
  dpi = 300,                    
  bg = "white"                    
)
grafico_random_BP<-grafico_random_ontologia(ic_pred_bp, "BP")
ggsave(
  filename = file.path(carpeta_resultados, "Top10GO_BP_Random_SCerevisiae.png"), 
  plot = grafico_random_BP, 
  width = 12,                    
  height = 8,                    
  dpi = 300,                    
  bg = "white"                    
)
grafico_random_CC<-grafico_random_ontologia(ic_pred_cc, "CC")
ggsave(
  filename = file.path(carpeta_resultados, "Top10GO_CC_Random_SCerevisiae.png"), 
  plot = grafico_random_CC, 
  width = 12,                    
  height = 8,                    
  dpi = 300,                    
  bg = "white"                    
